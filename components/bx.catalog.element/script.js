...
window.JCCatalogElement.prototype.SetProductMainPict = function(intPict)
{
	...
	if (0 < this.product.sliderCount)
	{
      ...
      if (!!RowItems && 0 < RowItems.length) {
        RowItems = BX.findChildren(this.product.slider.LIST, {tagName: 'option'}, false);
      }
...
( function($) {
  ...
  $( function() {
    //hide text inside color labels but not labels themselves
    //add modernizr check for css :checked support
    if (Modernizr.checked) {
      $('.item-colors + dd label').wrapInner('<span class="label_text"></span>');
      $('.item-colors + dd label .label_text').hide();
      $('.item-colors + dd label').each(function() {
        $(this).css('background-image', 'url("' + $(this).prev('input').data('color') + '")');
      });
    }
    ...
    //hide social nav links text
    $('.item-socialnav-icons a').wrapInner('<span class="link_text"></span>');
    $('.item-socialnav-icons .link_text').hide();
    $('.item-socialnav h1:first-child').wrapInner('<span></span>');
		
    //show dropdown value
    $( '.i-dropdown-select' ).delegate( 'li', 'click', function() {
      var $this = $( this ),
          text = $this.find( '.cnt' ).text();
